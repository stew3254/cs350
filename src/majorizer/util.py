...
def parse(file):
    ...
    for item in df:
        curr_time = str(item[2])
        if curr_time == "N/A":
            curr_time = -1
            end_time = begin_time = curr_time
        elif curr_time.startswith('L'):
            curr_time = curr_time[:2]
            time_array = filter(None, curr_time.split(" "))
        ...
        course_offering = DBCourseOffering(term=item[3], instructor=item[6], start_time=begin_time, end_time=end_time,
                                           days=days, room=item[6], section_num=1, course_id=course)
